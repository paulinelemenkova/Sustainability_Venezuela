...
\section{Study Area}
This study focuses on Venezuela\hl{, }one the of the most seismically active regions\hl{ }of \hl{South America. The spatial extent of the study area is} 59,5°-74°W, 0°-12.5°N\hl{ }(Figure \ref{fig1}). 


\begin{figure}[H]\centering
	\includegraphics[width=11.0 cm]{Fig_01}
	\caption{Topographic map of the\hl{ Venezuela region. }Mapping: GMT. Source: authors. \label{fig1}}
\end{figure}  
...
\begin{figure}[H]\centering
	...
	\caption{Geologic map of Venezuela\hl{ region. }Mapping: QGIS. Source: authors.\label{fig2}}
\end{figure}  
...
The north-western and north-eastern regions\hl{ of Venezuela are }prone to earthquakes \cite{Audemard,Perez} due to the tectonic activity of the lithosphere plates \citep{Barr,Hayes}. \hl{According to the Incorporated Research Institutions for Seismology (IRIS) database, the }repetitive occurrence of seismic events, such as volcanism and earthquakes in north-western\hl{ }Venezuela \hl{as well as} Trinidad and Tobago are recorded as \hl{a }series of\hl{ }events \hl{in }recent 25 years. The ongoing effects of earthquakes in Venezuela \citep{Audemard2006,Baumbach,Colon,Pousse2018,Rial,Yamazaki} and coastal regions of Trinidad and Tobago \citep{Deville,Latchman}, made geophysical mapping important task \hl{for} seismic hazard mapping, earthquake \hl{prognosis }and risk assessment \cite{Bucci}.

\begin{figure}[H]\centering
	\includegraphics[width=14.0 cm]{Fig_03}
	\caption{Geologic provinces in Venezuela\hl{ region. }Mapping: QGIS. Source: authors.\label{fig3}}
\end{figure} 

The Caribbean Sea region is notable for complex geologic setting (Figure \ref{fig3}) where several tectonic plates experience collision, interaction and subduction: Caribbean, Cocos, Nazca and South America plates. This results in \hl{a }high seismic activity in the region\hl{. The} influence of tectonic processes\hl{ is }reflected in \hl{a variety of topographic forms, geologic structure and geomorphic setting} of\hl{ }Venezuela\hl{, }as\hl{ }discussed \hl{earlier } \citep{Diebold,Fajardo,Lemenkova2020b,Perez1981}. The geodynamic heterogeneity of the upper mantle resulted in specific mountain geomorphology with notable features such as the extent of the Tobago Trough, Lara-Falcón Basin and crustal structure of the Cordillera de Mérida in southwestern Venezuela, as important segments of \hl{the }Andean orogeny \cite{AVILAGARCIA2022103853}. 
The distribution of earthquakes in this region is strongly controlled by the tectonic and geophysical setting, i.e., closeness to the margins of the colliding South America and Caribbean tectonic plates. Other\hl{ important factors include} mountain geomorphology and topographic variability shaped by the geophysical-geological forces which affect the variations in depth and magnitude of seismic events. 

\hl{Active} tectonic movements cause the formation of the orogenic mountain belt and topography. \hl{Besides, regional }bathymetry \hl{is }complicated \hl{by the }deep-sea trenches and troughs in the margins of the Pacific Ocean and the Caribbean Sea along the subduction path \citep{Gough,Lemenkova2020a}. Complex geological history is especially notable in the NW Venezuela, Falcón Basin, where\hl{ }interactions between the lithosphere plates and subducting Caribbean slab cause the associated seismic activity and geologic processes: \hl{the }outcrop of \hl{the }igneous intrusive bodies, crustal thinning, decreased Moho depth, gravity anomalies \hl{and} rifting \cite{Bezada}. \hl{Previous studies }\cite{Mazuera} identified crustal thinning beneath the Falcón Basin along the western extension of the Oca-Ancón fault system\hl{, }interpreted as a back-arc basin, and suture zones between the Proterozoic and Paleozoic provinces (Ouachita-Marathon-related suture), and Paleozoic and Meso-Cenozoic terranes (peri-Caribbean suture) where seismic velocity changes\hl{.} 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Methodology}

\subsection{Data}
The topographic, geophysical, seismic and geologic data\hl{ }were used \hl{from the open sources }to present a multi-disciplinary cartographic analysis of Venezuela. The topographic data were collected \hl{from }the open access repository \hl{of the }General Bathymetric Chart of the Oceans (GEBCO) \hl{in }NetCDF \hl{in }raster \hl{format with }grid \hl{cells }\cite{GEBCO}\hl{. The GEBCO }presents the most comprehensive data for topography and bathymetry of the Earth \hl{with 15 arc second resolution } \cite{Schenke}. The GEBCO data\hl{ are }widely used in geosciences due to its \hl{robustness: high }resolution\hl{, }precision\hl{ and reliability as a cartographic data source }\citep{Mcmichael,Cortina}. 

The geological data (Figures \ref{fig2} and \ref{fig3}) have been \hl{collected from the United States Geological Survey (USGS). The geologic maps were processed as .shp vector layers} using\hl{ }QGIS \hl{open source software due to its} compatibility\hl{ }with the ArcGIS data format. The visualization of these maps followed the traditional mapping process of the QGIS \cite{Lemenkova2022a}. In the first stage, vector layers of the geologic provinces and outcrops were visualized with in a Graphical User Interface (GUI). In the second stage, the study area was designated using the overlay of the Digital Chart of the World (DCW), which is a comprehensive 1:1,000,000 scale vector base map uploaded into the QGIS project. 

\hl{The geoid map is visualized} based on \hl{the data }collected from the EGM2008 \cite{Pavlis}. \hl{The free-air gravity maps and vertical gradient are based on the open source datasets available from the public repositories of Scripps Institution of Oceanography and described in} \cite{Sandwell}. \hl{The seismic dataset used for plotting map of earthquakes was obtained from the Incorporated Research Institutions for Seismology (IRIS) database, using spatial extent of Venezuela, neighbouring areas of Trinidad and Tobago and surroundings. The dataset covered the earthquake events in Venezuela region  for the period 1997 to 2021 and included earthquakes with magnitudes from 1.9} $M_L$ to 7.3 $M_L$, \hl{Richter scale.}

\subsection{Methods}
\hl{The }maps \hl{showing }topographic, seismic and geophysical \hl{setting of Venezuela }(Figures \ref{fig1}, \ref{fig4}, \ref{fig5}, \ref{fig6} and \ref{fig7} were made using scripting techniques\hl{ }of\hl{ }GMT \cite{Wessel} following the existing \hl{described methodology }\citep{Lemenkova2022b,Lemenkova202217}. The methodological workflow included \hl{five }algorithms by GMT codes \hl{which are presented in the Appendix section of this article.} Prior to data visualization and mapping, the\hl{ scripts }were written using \hl{GMT} syntax and \hl{organised }in the Xcode environment. \hl{As mentioned in the previous subsection, we processed high-resolution data using GMT. Conventionally, the }language of GMT was used \hl{in several modules for computing and processing raster data }for plotting\hl{ }cartographic elements. \hl{The} area of interest was first selected and cut from the GEBCO grid using the following \hl{snippet of }code: $'gmt grdcut GEBCO\_2019.nc -R286/300.5/0/12.5 -Gve\_relief.nc'.$

Following the first step, all other the substantial cartographic elements\hl{ }were added in a script using the following GMT modules: psbasemap, grdimage, psscale, pstext, psclip, grdcontour. \hl{Specifically, these include} images visualized in selected colour palettes, grids, clipped insert region\hl{ }in\hl{ a }globe map, coastlines, borders, rivers, bar legends \hl{on the maps}, scale bars\hl{, }text annotations, time stamp\hl{, etc. }The technique of scripting was applied from the existing detailed descriptions \cite{Lemenkova2021b}. For example, the 'grdcontour' module was used for computing, modelling and visualizing topographic isolines using quantitative DEM data as follows: $'gmt grdcontour ve\_relief1.nc -R -J -C1000 -Wthinner,darkbrown -O -K >> \$ps'$. The purpose of the geoid visualization (Figure \ref{fig4}) in relation to geophysical differentiation was to investigate how gravity values respond to the effects of topographic elevation and bathymetric depressions. 

Whilst GEBCO data were gathered in NetCDF format ($GEBCO\_2019.nc$), the geoid data needed to be converted first from the .adf format\hl{. This }was done by the following code of GMT: $'gmt grdconvert n00w90/w001001.adf geoid\_02.grd'.$ The data extent and range were examined by \hl{Geospatial Data Abstraction Library }(GDAL) and interpreted for the colour palette as follows: $'gmt makecpt -C33_blue\_red.cpt -T-55/25 > colors.cpt'.$ The geoid was modelled using the following code: $'gmt grdimage geoid\_02.grd -Ccolors.cpt -R286/300.5/0/12.5 -JM6.5i -P -Xc -I+a15+ne0.75 -K > \$ps'$. \hl{The visualzied data are shown in Figure} \ref{fig4}. 

\begin{figure}[H]\centering
	\includegraphics[width=12.0 cm]{Fig_04}
	\caption{Geoid model over the Venezuela\hl{ region. }Mapping: GMT. Source: authors.\label{fig4}}
\end{figure} 

The gravity grids and \hl{the }geophysical data (Figure \ref{fig5} and \ref{fig6}) were also converted from the initial raw IMG format to the GRD format \hl{which is compatible} with the GMT\hl{. The code was executed }as follows: $'gmt img2grd grav\_27.1.img -R286/300.5/0/12.5 -Ggrav\_VE.grd -T1 -I1 -E -S0.1 -V'$ (here for \hl{Figure }\ref{fig5}). The same procedure has been repeated for\hl{ Figure }\ref{fig6} (vertical gradient of gravity)\hl{, following }the examples\hl{ described in the existing }work \cite{Lemenkova2021d}. The annotations have been added on all the maps using the 'pstext' module of GMT as in the following example: $'gmt pstext -R -J -N -O -K -F+f10p,0,black+jLB+a-0 >> \$ps << EOF 294.20 8.24 Ciudad Bolívar EOF'$\hl{. }The data were converted to \hl{the }‘ngdc’ format, visualized by 'psxy' module \hl{and mapped in Figure} \ref{fig5}. 

The earthquakes \hl{in }Figure \ref{fig7} (map of seismicity) have been plotted using the 'psxy' module of GMT by the following code: $'gmt psxy -R -J quakes\_VE.ngdc -Wfaint -i4,3,6,6s0.05 -h3 -Scc -Csteps.cpt -O -K >> \$ps'.$ Here the ‘i4,3,6,6s0.05’ means the numbers of column in the table ($quakes\_VE.ngdc$) that was converted from the original CSV from the IRIS. The volcanoes were visualized by the following code: $‘gmt psxy -R -J volcanoes.gmt -St0.4c -Gred -Wthinnest -O -K >> \$ps’$. The complex legend showing the magnitude of events was plotted using the following code: $‘gmt pslegend -R -J -Dx1.5/-3.0+w17.8c+o-2.0/0.1c  -F+pthin+ithinner+gwhite -O -K << FIN >> \$ps H 10 Helvetica Seismicity: earthquakes magnitude (M) from \\1.9 to 7.3 N 9 S 0.3c c 0.3c red 0.01c 0.5c M (7.1-7.3) <...> FIN’$.

\begin{figure}[H]\centering
	\includegraphics[width=12.0 cm]{Fig_05}
	\caption{Free-air gravity model of the Venezuela\hl{ region. }Mapping: GMT. Source: authors.\label{fig5}}
\end{figure} 

\hl{In practice, we fused the data from seismic events to the topographic grid to show the locations of the events on the map with their attributes showing magnitude ranging from 1.9 to 7.3. In our maps we demonstrated that the density of the earthquake events visibly increases along the line of lithosphere plate boundaries (purple thick line on the map in Figure)} \ref{fig7}\hl{ with maximal concentration of the events in the Andes (Cordillera de Merida) and the Trinidad and Tobago region}. 
...
Figure \ref{fig1} has been plotted using GEBCO raster grid selected due to its quality and reliability.\hl{ The }precise topographic\hl{ map is essential to a fundamental }understanding of geophysics and tectonics \hl{of the region. Topographic maps enable to analyse earthquake location and evaluate the }associated effects on ocean and terrestrial geomorphology. \hl{Therefore, the analysis of the topography is necessary for} mapping seismicity, risk assessment and tsunami wave propagation. Here the topographic and bathymetric elevations of the study area encompassing Venezuela, Trinidad and Tobago islands range from -4948 m to 5536 m\hl{, }according to the GEBCO grid. 

\begin{figure}[H]\centering
	\includegraphics[width=12.0 cm]{Fig_06}
	\caption{Vertical gradient of gravity in the Venezuela\hl{ region. }Mapping: GMT. Source: authors.\label{fig6}}
\end{figure}  

Figure \ref{fig2} and \ref{fig3} indicate spatial extent of geological provinces and outcrops in Venezuela. The map in Figure \ref{fig2} shows that most of the territory of Venezuela is covered by \hl{the }Guyana Shield (aquamarine colour) in the south-eastern parts of the country. The Barinas-Apure sedimentary basin is located in\hl{ }Llanos\hl{ }and the Andean foothills region\hl{, }western Venezuela \cite{Callejon}. The Barinas-Apure basin (yellow colour in the map) includes oil fields\hl{, which comprise }the 3rd most important producing area in Venezuela after the Maracaibo and Eastern Venezuela basins\hl{, }according to the petroleum accumulation. The Falcón sedimentary basin, which produces indigenous Miocene oil from structural and stratigraphic accumulations \cite{Rodriguez}\hl{, }is coloured slate blue in the map. 

The South Caribbean deformed belt (purple colour in Figure \ref{fig2}) represents a submarine prism formed between the subducting tectonic plates in the Colombian and Venezuelan basins and arc terranes along the northern rim of South America \cite{Mann}. Its \hl{southwestern} part, the Sinu fold belt\hl{ }forms an accretionary wedge in the \hl{area }of\hl{ }subduction of the Caribbean Plate under \hl{the }South American Plate \cite{Rodriguez2021}. Other important geological provinces include the Guyana Basin which stretches along the passive margin of \hl{the northeastern} South America and the Maracaibo Basin (green, Figure \ref{fig2}). The Cariaco Basin (beige colour in Figure \ref{fig2}) is an East-West stretching basin \cite{Schubert} situated in the Gulf of Cariaco, \hl{located }on the continental shelf of the Caribbean Sea, off the eastern coast \hl{in the surroundings of }Barcelona,\hl{ }Figure \ref{fig1}). 

The Tobago Trough (chartreuse green colour in Figure \ref{fig2}) is a modern marine forearc basin of the Lesser Antilles arc located above the sedimentary succession\hl{. It is }at least 10 km thick and probable oceanic basement\hl{, }formed by sediments from various stratigraphic sequences: Quaternary, early Pleistocene -- late Miocene, Miocene and late Eocene\hl{. Besides, it }may also include \hl{the }mid-Cretaceous or older \hl{intrusive suites }\cite{Speed}. The \hl{distribution of the }Quaternary sediments covering the most of the north-west of the country (dark pink colour in Figure \ref{fig3}) correspond\hl{ }with the previous studies on Plio-Quaternary extension in the Venezuelan Andes\hl{. Thus, they }show\hl{ }the extensional structures corresponding to the elongated tilted blocks with geometry and kinematics of the structures corresponding to the earlier syn-orogenic extension \cite{Dhont}. 

\begin{figure}[H]\centering
	\includegraphics[width=12.0 cm]{Fig_07}
	\caption{Seismic map of earthquakes in the region of Venezuela\hl{. }Mapping: GMT. Source: authors.\label{fig7}}
\end{figure}   

Figure \ref{fig4} show the variations of the geoid heights over the study area. The highest values of geoid (up to 25 m, bright red \hl{colours }are visible in the south-western region of the map \hl{on the border with }Colombia\hl{. However, }the majority of \hl{the territory of }Venezuela covers the extent of -50 to 8 m with the highest values in the mountains\hl{, which shows a }clearly visible correlation with topographic elevations. The undulations of geoid in the Guyana Highlands corresponds to the higher topographic elevations on the shield as table-like mountains ‘tepuis’. The marine areas of the Caribbean Sea have in general lower geoid heights (blue colored areas in Figure \ref{fig4}) which well corresponds to the bathymetric depressions reflected in the geophysical setting of the Earth.

\hl{ The comparison of the }geoid mode in Figure \ref{fig4} with topographic and geologic maps in Figure \ref{fig1} and \ref{fig2}\hl{ shows }the elevated values of geoid\hl{ which }are notable in the region of the Guyana Shield. The Guyana Shield is a Precambrian geological formation in the NE coast of the South America and \hl{is }an area of special importance in historical geology of Venezuela due to \hl{the }high potential for mineral exploration \cite{Gibbs}. \hl{Besides the geologic features, the }environmental importance of this unique region consists in the highest biodiversity in the world\hl{ which includes numerous }endemic species \cite{Hollowell} \hl{of }tropical forests of Amazonia \cite{Hammond}. 

Figure \ref{fig5} shows variations in the gravity over the study area. The notable free-air gravity low coincides with distribution of the Guyana basin, \hl{southeastern} part of the Venezuela\hl{ and the }Trinidad and Tobago\hl{. In particular, this }points at the tectonic basement depression formed during \hl{the }Jurassic rifting, \hl{as well as }Lake Maracaibo, an important geologic region of Venezuela. The \hl{northeastern }coasts of the Lake Maracaibo are occupied by the Bolivar Coastal Fields, the largest oil field in South America\hl{. With with basin area of 50,000 km2 and oil production of 30 *109 bbl, it is }the 2nd oil field\hl{ among the }most prolific hydrocarbon basins in the world after the Middle Eastern\hl{ hydrocarbon basins }\cite{Escalona}.   

The Maracaibo Lake has a wide variety of oil reservoir rocks with structural traps in the tectonic units, e.g.\hl{, }normal or inverted faults on the South American plate \cite{Bockmeulen}. The oil formed in the Maracaibo basin is mostly heavy black oil with asphaltene deposition \cite{Escobar,Memon}. The gravity in the Lake Maracaibo reach below -200 mGal correlating to the bathymetric depression (Figure \ref{fig1}) and geologic extend of the Maracaibo Basin (Figure \ref{fig2}).\hl{ The }highest values for gravity reach +200 mGal and well corresponds to the topographic elevation of the Cordillera de Mérida, Columbian Andes, Guiana Highlands and Sierra Parima. The Amazon and Orinoco Flow basin mostly have gentle fluctuations of gravity with values in the range between -25 to 30 mGal (Figure \ref{fig5}).
